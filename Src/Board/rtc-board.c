...
void RtcBackupWrite(uint32_t data, uint32_t data_type)
{
	switch (data_type)
	{
		...
		case 19:
			...
			break;
		case 20:
			HAL_RTCEx_BKUPWrite(&RtcHandle, RTC_BKP_DR20, data);
			break;
		case 21:
			HAL_RTCEx_BKUPWrite(&RtcHandle, RTC_BKP_DR21, data);
			break;
		default:
			break;
	}
...
uint32_t RtcBackupRead(uint32_t data_type)
{
	switch (data_type)
	{
		...
		case 20:
			return (HAL_RTCEx_BKUPRead(&RtcHandle, RTC_BKP_DR20));
		case 21:
			return (HAL_RTCEx_BKUPRead(&RtcHandle, RTC_BKP_DR21));
		default:
			return 0x0;
	}
}
