...
void I2cMcuInit( I2c_t *obj, I2cId_t i2cId, PinNames scl, PinNames sda )
{
    ...
    GpioInit( &obj->Scl, scl, PIN_ALTERNATE_FCT, PIN_OPEN_DRAIN, PIN_PULL_UP, GPIO_AF4_I2C1 );
    GpioInit( &obj->Sda, sda, PIN_ALTERNATE_FCT, PIN_OPEN_DRAIN, PIN_PULL_UP, GPIO_AF4_I2C1 );
...
//        I2cHandle.Init.DutyCycle = I2C_DUTYCYCLE_16_9;
    ...
    I2cHandle.Init.OwnAddress1 = 164;
...
//    status = ( HAL_I2C_Mem_Write( &I2cHandle, deviceAddr, addr, memAddSize, buffer, size, 2000 ) == HAL_OK ) ? SUCCESS : FAIL;
    status = (HAL_I2C_Master_Transmit(&I2cHandle, deviceAddr, ucTemp, memAddSize+size, 2000) == HAL_OK ) ? SUCCESS : FAIL;
    return status;
}
...
//    status = ( HAL_I2C_Mem_Read( &I2cHandle, deviceAddr, addr, memAddSize, buffer, size, 2000 ) == HAL_OK ) ? SUCCESS : FAIL;
    status = ( HAL_I2C_Master_Transmit(&I2cHandle, deviceAddr, &addr, memAddSize, 2000) == HAL_OK ) ? SUCCESS : FAIL;
    if(status == SUCCESS)
    {
