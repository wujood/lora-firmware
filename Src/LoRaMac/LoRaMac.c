...
typedef enum eLoRaMacRequestHandling {
	LORAMAC_REQUEST_HANDLING_OFF = 0, LORAMAC_REQUEST_HANDLING_ON = !LORAMAC_REQUEST_HANDLING_OFF
...
static void OnRadioRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr)
{
	...
	RtcBackupWrite(RxDoneParams.LastRxDone, LastRxDone);
//    RtcBackupWrite(RxDoneParams.Payload, Payload);
	RtcBackupWrite(RxDoneParams.Rssi, Rssi);
	RtcBackupWrite(RxDoneParams.Snr, Snr);
...
//                RtcBackupWrite(  MacCtx.NvmCtx->NetID , NetID );

// Device Address
				MacCtx.NvmCtx->DevAddr = macMsgJoinAccept.DevAddr;

//                RtcBackupWrite(  MacCtx.NvmCtx->DevAddr , DevAddr );
...
//                RtcBackupWrite(  MacCtx.NvmCtx->MacParams.Rx1DrOffset , Rx1DrOffset );
//                RtcBackupWrite(  MacCtx.NvmCtx->MacParams.Rx2Channel.Datarate , Rx2Channel_Datarate );
//                RtcBackupWrite(  MacCtx.NvmCtx->MacParams.RxCChannel.Datarate , RxCChannel_Datarate );
...
//                RtcBackupWrite(  MacCtx.NvmCtx->MacParams.ReceiveDelay1 , ReceiveDelay1 );
//                RtcBackupWrite(  MacCtx.NvmCtx->MacParams.ReceiveDelay2 , ReceiveDelay2 );
				...
				RtcBackupWrite(MacCtx.NvmCtx->NetworkActivation, NetworkActivation);
		...
		case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
			...
			fCntHandlerStatus = LoRaMacGetFCntDown(addrID, fType, &macMsgData, MacCtx.NvmCtx->Version, phyParam.Value,
					&fCntID, &downLinkCounter);
			...
			switch (fType)
			{
				 ...
				 */
					...
					ProcessMacCommands(macMsgData.FHDR.FOpts, 0, macMsgData.FHDR.FCtrl.Bits.FOptsLen, snr,
				 ...
				 */
					...
					ProcessMacCommands(macMsgData.FRMPayload, 0, macMsgData.FRMPayloadSize, snr,
							MacCtx.McpsIndication.RxSlot);
					MacCtx.McpsIndication.Port = macMsgData.FPort;
					break;
				}
...
static void HandleRadioRxErrorTimeout(LoRaMacEventInfoStatus_t rx1EventInfoStatus,
		LoRaMacEventInfoStatus_t rx2EventInfoStatus)
{
	bool classBRx = false;
...
static uint8_t LoRaMacCheckForTxTimeout(void)
{
	if ((LoRaMacConfirmQueueGetStatusCmn() == LORAMAC_EVENT_INFO_STATUS_TX_TIMEOUT) || (MacCtx.McpsConfirm.Status == LORAMAC_EVENT_INFO_STATUS_TX_TIMEOUT))
	{
		...
		case CLASS_A:
		{
			...
			if (deviceClass == CLASS_C)
			{
				...
				for (int8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++)
				{
					...
					{
						MacCtx.NvmCtx->MacParams.RxCChannel.Frequency =
								MacCtx.NvmCtx->MulticastChannelList[i].ChannelParams.RxParams.ClassC.Frequency;
						MacCtx.NvmCtx->MacParams.RxCChannel.Datarate =
								MacCtx.NvmCtx->MulticastChannelList[i].ChannelParams.RxParams.ClassC.Datarate;
				...
				RegionComputeRxWindowParameters(MacCtx.NvmCtx->Region, MacCtx.NvmCtx->MacParams.RxCChannel.Datarate,
						MacCtx.NvmCtx->MacParams.MinRxSymbols, MacCtx.NvmCtx->MacParams.SystemMaxRxError,
		...
		switch (payload[macIndex++])
		{
			...
			case SRV_MAC_LINK_ADR_REQ:
			{
				...
				if (adrBlockFound == false)
				{
					...
					status = RegionLinkAdrReq(MacCtx.NvmCtx->Region, &linkAdrReq, &linkAdrDatarate, &linkAdrTxPower,
							&linkAdrNbRep, &linkAdrNbBytesParsed);
			...
			case SRV_MAC_TX_PARAM_SETUP_REQ:
			{
				...
				if (RegionTxParamSetupReq(MacCtx.NvmCtx->Region, &txParamSetupReq) != -1)
				{
					...
					getPhy.UplinkDwellTime = MacCtx.NvmCtx->MacParams.UplinkDwellTime;
					phyParam = RegionGetPhyParam(MacCtx.NvmCtx->Region, &getPhy);
					MacCtx.NvmCtx->MacParams.ChannelsDatarate = MAX(MacCtx.NvmCtx->MacParams.ChannelsDatarate,
...
static LoRaMacStatus_t ScheduleTx( bool allowDelayedTx)
{
	...
	// Select channel
	status = RegionNextChannel(MacCtx.NvmCtx->Region, &nextChan, &MacCtx.Channel, &dutyCycleTimeOff,
			&MacCtx.NvmCtx->AggregatedTimeOff);
	...
	RegionComputeRxWindowParameters(MacCtx.NvmCtx->Region,
			RegionApplyDrOffset(MacCtx.NvmCtx->Region, MacCtx.NvmCtx->MacParams.DownlinkDwellTime,
					MacCtx.NvmCtx->MacParams.ChannelsDatarate, MacCtx.NvmCtx->MacParams.Rx1DrOffset),
			MacCtx.NvmCtx->MacParams.MinRxSymbols, MacCtx.NvmCtx->MacParams.SystemMaxRxError, &MacCtx.RxWindow1Config);
	// Compute Rx2 windows parameters
	RegionComputeRxWindowParameters(MacCtx.NvmCtx->Region, MacCtx.NvmCtx->MacParams.Rx2Channel.Datarate,
			MacCtx.NvmCtx->MacParams.MinRxSymbols, MacCtx.NvmCtx->MacParams.SystemMaxRxError, &MacCtx.RxWindow2Config);
			...
			if (macCmdsSize > 0)
			{
				...
				if ((MacCtx.AppDataSize > 0) && (macCmdsSize <= LORA_MAC_COMMAND_MAX_FOPTS_LENGTH))
				{
					if (LoRaMacCommandsSerializeCmds( LORA_MAC_COMMAND_MAX_FOPTS_LENGTH, &macCmdsSize,
							MacCtx.TxMsg.Message.Data.FHDR.FOpts) != LORAMAC_COMMANDS_SUCCESS)
					{
						return LORAMAC_STATUS_MAC_COMMAD_ERROR;
					}
...
LoRaMacStatus_t LoRaMacMlmeRequest(MlmeReq_t* mlmeRequest)
{
	...
	switch (mlmeRequest->Type)
	{
		case MLME_JOIN:
		{
			...
			if ((mlmeRequest->Req.Join.DevEui == NULL) || (mlmeRequest->Req.Join.JoinEui == NULL))
			{
				return LORAMAC_STATUS_PARAMETER_INVALID;
			}
			...
			MacCtx.NvmCtx->MacParams.ChannelsDatarate = RegionAlternateDr(MacCtx.NvmCtx->Region,
					mlmeRequest->Req.Join.Datarate, ALTERNATE_DR);
			...
			if (status != LORAMAC_STATUS_OK)
			{
				...
				MacCtx.NvmCtx->MacParams.ChannelsDatarate = RegionAlternateDr(MacCtx.NvmCtx->Region,
						mlmeRequest->Req.Join.Datarate, ALTERNATE_DR_RESTORE);
			}
			break;
		}
		...
		case MLME_TXCW_1:
		{
			...
			status = SetTxContinuousWave1(mlmeRequest->Req.TxCw.Timeout, mlmeRequest->Req.TxCw.Frequency,
...
{
	RxDoneParams.LastRxDone = RtcBackupRead(LastRxDone);
	//    RtcBackupRead(RxDoneParams.Payload, Payload);
	RxDoneParams.Size = 33;
	RxDoneParams.Rssi = RtcBackupRead(Rssi);
	RxDoneParams.Snr = RtcBackupRead(Snr);
}
...
//	macMsgJoinAccept.Buffer = payload;
	...
	macCryptoStatus = LoRaMacCryptoRestoreJoinAccept(JOIN_REQ, MacCtx.JoinEui, &macMsgJoinAccept);
...
{
	RtcBackupWrite(0x00, MHDRValue);
	RtcBackupWrite(0x00, JoinNonce);
	RtcBackupWrite(0x00, NetID);
	RtcBackupWrite(0x00, DevAddr);
	RtcBackupWrite(0x00, DLSettings);
	RtcBackupWrite(0x00, RxDelay);
	RtcBackupWrite(0x00, MIC);
	RtcBackupWrite(0x00, FCntUp);
	RtcBackupWrite(0x00, DevNonce);
	RtcBackupWrite(0x00, NetworkActivation);
	RtcBackupWrite(0x00, CFList1);
	RtcBackupWrite(0x00, CFList2);
	RtcBackupWrite(0x00, CFList3);
	RtcBackupWrite(0x00, CFList4);
	RtcBackupWrite(0x00, LastRxDone);
	RtcBackupWrite(0x00, Payload);
	RtcBackupWrite(0x00, Size);
	RtcBackupWrite(0x00, Rssi);
	RtcBackupWrite(0x00, Snr);
}

uint8_t GetNetworkActivation(void)
{

	MacCtx.NvmCtx->NetworkActivation = RtcBackupRead(NetworkActivation);
