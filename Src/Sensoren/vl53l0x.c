...
unsigned char ucDefTuning[] = {	80, 0xff, 0x01, 0x00, 0x00, 0xff, 0x00, 0x09, 0x00, 0x10, 0x00, 0x11, 0x00, 0x24, 0x01, 0x25, 0xff,
...
static void getSequenceStepTimeouts(struct vl53l0x_dev *dev, uint8_t enables, SequenceStepTimeouts * timeouts)
{
	...
	timeouts->pre_range_mclks = decodeTimeout(readReg16(dev, VL53L0X_REG_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
	...
	timeouts->final_range_mclks = decodeTimeout(readReg16(dev, VL53L0X_REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI));
...
int setVcselPulsePeriod(struct vl53l0x_dev *dev, vcselPeriodType type, uint8_t period_pclks)
{
	...
	if (type == VcselPeriodPreRange)
	{
		...
		writeReg16(dev, VL53L0X_REG_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI, encodeTimeout(new_pre_range_timeout_mclks));
	...
	else if (type == VcselPeriodFinalRange)
	{
		...
		writeReg16(dev, VL53L0X_REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI, encodeTimeout(new_final_range_timeout_mclks));
...
int setMeasurementTimingBudget(struct vl53l0x_dev *dev, uint32_t budget_us)
{
	...
	if (enables & SEQUENCE_ENABLE_FINAL_RANGE)
	{
		...
		writeReg16(dev, VL53L0X_REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI, encodeTimeout(final_range_timeout_mclks));
